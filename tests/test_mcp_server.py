        ...
        try:
            return await client.request(
                ...
                headers={
                    ...
                    "Cf-Access-Jwt-Assertion": "untrusted-caller-assertion",
                },
            )
        finally:
            await client.aclose()
...
def test_panel_api_client_forwards_only_request_scoped_access_assertion():
    received: dict[str, str | None] = {}
    assertion = "trusted-request-scoped-assertion"

    def handler(request: httpx.Request) -> httpx.Response:
        received["authorization"] = request.headers.get("Authorization")
        received["access_assertion"] = request.headers.get("Cf-Access-Jwt-Assertion")
        return httpx.Response(200, json={"ok": True})

    async def run():
        client = PanelApiClient(_managed_config(), transport=httpx.MockTransport(handler))
        token = mcp_server._MCP_REQUEST_CONTEXT.set(
            McpRequestContext(
        ...
        try:
            return await client.get("/api/tasks", {"workspace": "workspace-a"})
        finally:
            mcp_server._MCP_REQUEST_CONTEXT.reset(token)
            await client.aclose()
...
def test_all_task_scoped_mcp_tools_forward_workspace():
    ...
    task_scoped_tools = {
        "scaffold_task_list",
        "scaffold_task_detail",
        "scaffold_task_draft_detail",
        "scaffold_task_check",
        "scaffold_import_list",
        "scaffold_import_detail",
        "scaffold_data_lake_preview",
        "scaffold_data_lake_import_dry_run",
        "scaffold_job_status",
    }
    for name in task_scoped_tools:
        assert "workspace" in tools[name].inputSchema["required"]
    asyncio.run(
        server.call_tool(
            "scaffold_task_draft_detail",
...
@pytest.mark.parametrize(
    ...
    [
        ("scaffold_task_list", {}),
        ("scaffold_task_detail", {"task_id": "task-a"}),
        ("scaffold_task_draft_detail", {"task_id": "task-a"}),
        ("scaffold_task_check", {"task_id": "task-a"}),
        ("scaffold_import_list", {"task_id": "task-a"}),
        ("scaffold_import_detail", {"task_id": "task-a", "import_id": "import-a"}),
        ("scaffold_data_lake_preview", {"task_id": "task-a"}),
        ("scaffold_data_lake_import_dry_run", {"task_id": "task-a"}),
        ("scaffold_job_status", {"task_id": "task-a"}),
    ],
)
def test_task_scoped_mcp_tools_require_workspace(tool_name: str, arguments: dict[str, Any]):
    server = create_mcp_server(_config(), panel_client=FakePanelClient())
...
def test_task_scoped_mcp_write_tools_require_workspace(tool_name: str, arguments: dict[str, Any]):
    server = create_mcp_server(_config(enable_writes=True), panel_client=FakePanelClient())
...
def test_mcp_tools_do_not_serialize_concurrent_panel_requests():
    ...
    async def run():
        ...
        await asyncio.gather(
            server.call_tool("scaffold_task_list", {"workspace": "workspace-b"}),
            server.call_tool("scaffold_task_list", {"workspace": "workspace-a"}),
        )
...
def test_panel_api_errors_redact_configured_tokens():
    assertion = "sensitive-delegated-access-assertion"

    async def handler(_: httpx.Request) -> httpx.Response:
        return httpx.Response(
            500,
            json={"error": f"echo {INTERNAL_TOKEN}, {MCP_TOKEN}, and {assertion}"},
        )
