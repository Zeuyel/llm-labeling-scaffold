...
class TaskRef:
    ...
    lifecycle_state: TaskLifecycle = TaskLifecycle.ACTIVE
...
class TaskLifecycleConflict(RuntimeError):
    ...
    def __init__(self, current_state: TaskLifecycle, requested_state: TaskLifecycle):
...
class DatabaseService:
    ...
    def disable_task(
        ...
        *,
        ...
        return self.transition_task_lifecycle(
            actor_identity=actor_identity,
            caller_identity=caller_identity,
            workspace_slug=workspace_slug,
            task_key=task_key,
            target_state=TaskLifecycle.DISABLED,
            reason=reason,
            idempotency_key=idempotency_key,
            channel=channel,
            request_id=request_id,
    ...
    def archive_task(
        ...
        *,
        ...
        return self.transition_task_lifecycle(
            ...
            workspace_slug=workspace_slug,
            task_key=task_key,
            target_state=TaskLifecycle.ARCHIVED,
            reason=reason,
            idempotency_key=idempotency_key,
            channel=channel,
            request_id=request_id,
    ...
    def restore_task(
        ...
        *,
        ...
        return self.transition_task_lifecycle(
            ...
            workspace_slug=workspace_slug,
            task_key=task_key,
            target_state=TaskLifecycle.ACTIVE,
            reason=reason,
            idempotency_key=idempotency_key,
            channel=channel,
            request_id=request_id,
    ...
    def list_authorized_tasks(
        ...
        *,
        ...
        if not include_archived:
            task_query = task_query.where(Task.lifecycle_state != TaskLifecycle.ARCHIVED)
    ...
    def transition_task_lifecycle(
        ...
        *,
        ...
        try:
            target_state = (
                target_state
                if isinstance(target_state, TaskLifecycle)
                else TaskLifecycle(str(target_state))
            )
        except ValueError as exc:
            raise ValueError("target_state must be active, disabled, or archived") from exc
        ...
        if not _task_lifecycle_transition_allowed(previous_state, target_state):
            raise TaskLifecycleConflict(previous_state, target_state)
        ...
        audit_event = append_audit_event(
            self._session,
            workspace_id=task.workspace_id,
            event_type="task.lifecycle_changed",
            actor=actor,
            caller=caller,
            channel=channel,
            resource_type="task",
    ...
    *,
    ...
    try:
        return TaskLifecycleResult(
            ...
            task_id=uuid.UUID(body["task_id"]),
            task_key=str(body["task_key"]),
            previous_state=TaskLifecycle(body["previous_state"]),
            lifecycle_state=TaskLifecycle(body["lifecycle_state"]),
            reason=str(body["reason"]),
            changed=bool(body["changed"]),
            audit_event_id=uuid.UUID(body["audit_event_id"]),
...
def _task_lifecycle_transition_allowed(
    current_state: TaskLifecycle,
    target_state: TaskLifecycle,
) -> bool:
    allowed_targets = {
        TaskLifecycle.ACTIVE: {TaskLifecycle.ACTIVE, TaskLifecycle.DISABLED, TaskLifecycle.ARCHIVED},
        TaskLifecycle.DISABLED: {TaskLifecycle.ACTIVE, TaskLifecycle.DISABLED, TaskLifecycle.ARCHIVED},
        TaskLifecycle.ARCHIVED: {TaskLifecycle.ACTIVE, TaskLifecycle.ARCHIVED},
    }
    return target_state in allowed_targets[current_state]
