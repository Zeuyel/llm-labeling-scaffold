...
MCP endpoint 为 `http://localhost:8766/mcp`，生产只通过 Cloudflare Tunnel 暴露对应 HTTPS hostname。Cloudflare Edge 处理 OAuth discovery、动态客户端注册、Authorization Code + PKCE、opaque access token 和 refresh token；源站只验证 `Cf-Access-Jwt-Assertion`，不会实现 `/authorize`、`/token` 或保存 OAuth token/client secret。MCP 将已验证的 request-scoped assertion 与内部 service bearer 一起发送到 Panel：assertion 确定用户 actor，bearer 只确定 service caller，数据库 RBAC 始终按 actor 和显式 workspace 判权。MCP 不持有 Panel 管理员密码，也不直接挂载 R2 或本地数据目录。默认只注册只读工具；确需创建草稿、发布任务或提交数据湖导入时，才在受控部署中设置 `LLS_MCP_ENABLE_WRITES=1`。按用户过滤 tool list/execution 仍由 #47 负责，完整边界见 [MCP 接入说明](docs/mcp_integration.md)。
