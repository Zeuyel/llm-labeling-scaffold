...
`.env.example` 的 `basic_dev` 只服务于本地快速启动，并且只能与 `LLS_DEPLOYMENT_MODE=loopback` 组合。生产 sidecar Tunnel 设置 `LLS_DEPLOYMENT_MODE=tunnel` 和 `LLS_PANEL_AUTH_MODE=cloudflare_access`；base Compose 保留容器内 `0.0.0.0` 供 Docker network 访问，但不创建宿主机 Panel 端口。宿主机运行 Tunnel 时可使用 loopback override，不得开放非 loopback Panel 端口。
