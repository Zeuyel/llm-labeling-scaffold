...
| `scaffold_data_lake_import` dry-run | `POST /api/import/data_lake` with `{"task_id":"<task_id>","import_id":"<optional>","dry_run":true}`，返回 task/source/manifest 摘要、import id 和 validation |
...
所有推荐 CLI 命令成功时都在 stdout 输出 JSON。MCP 应把非零退出码视为调用失败，并把 stderr/stdout 摘要返回给操作者，不应自动重试会写入本地资产的导入命令，除非 import ID 和数据湖血缘完全相同。通过面板 API 发起 R2 导入时，MCP 应先调用 dry-run；真实 submit 必须携带 `confirm: true` 和稳定 `idempotency_key`。MCP 应记录返回的 job，并轮询 job 状态；导入成功后再触发 profile 的样本抽取阶段。
