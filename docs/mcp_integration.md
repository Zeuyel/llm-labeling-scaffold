...
MCP companion 通过 Docker 内网访问 `http://panel:8765`。每个已验证请求都把 request-scoped Access assertion 与内部 bearer 一起发送到 Panel：assertion 确定用户 actor，bearer 只确定 `mcp` service caller。companion 不持有 Panel 管理员密码，也不挂载 R2 凭据、`runs/` 或 `tasks/`。需要写工具时，将 Panel 与 MCP 的 `LLS_MCP_ENABLE_WRITES` 显式改为 `1` 后强制重建容器。
...
MCP middleware 使用 MCP application 的 issuer/AUD 验证 assertion，并建立请求级 `(issuer, subject)` context。`PanelApiClient` 只能从该 context 读取 assertion，不能信任 tool 参数或调用方自带的 actor/header。Panel 使用 MCP 专用 verifier 再次验证 actor，并用内部 bearer 确定 service caller；数据库 RBAC 始终按 actor 检查，审计分别记录真实 actor、caller=`mcp` 和服务端推导的 `channel=mcp`。内部 bearer 单独调用业务路由、缺失/无效 assertion、Panel/MCP AUD 混用都会 fail closed。

所有 task-scoped tool 都必须显式提交 `workspace`，MCP 不使用 Panel 的单 workspace 兼容回退。control mode 下 imports、jobs 和数据湖导入只访问 `runs/<workspace>/<task>/...`，并在访问前执行数据库 task ACL。tool listing/execution 的按用户过滤仍由 #47 负责。
...
| `scaffold_task_draft_update` | 写入 | update draft | 仅启用写操作时注册；必须提交当前 `expected_draft_etag`。 |
| `scaffold_task_publish` | 写入 | publish | 仅启用写操作时注册；必须提交 `expected_draft_etag`、reason、`confirm=true` 与稳定 `idempotency_key`。 |
...
任务草稿详情通过响应头和 DTO 返回完整强 ETag。更新和发布必须把该值原样作为 `expected_draft_etag`/`If-Match` 提交；缺失返回 `428 Precondition Required`，草稿已变化返回 `412 Precondition Failed`，不会静默覆盖。同一个发布 key 和同一草稿会返回原 revision；同一个 key 作用于不同草稿会拒绝。
...
MCP server 的测试覆盖：独立 issuer/AUD、签名与时间/type/sub 声明、Panel AUD 混用、并发 context 隔离、外部 Authorization 剥离、request-scoped assertion 转发、Panel 内部 token 固定、敏感值不泄漏、workspace 必填、runtime namespace、真实 actor/caller 审计、生产 fail closed、`static_dev` 回环限制和工具白名单。部署后可检查：
