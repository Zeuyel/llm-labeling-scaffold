...
- 平台成员的列表、邀请、角色变更和撤销由 `/api/members` 及 `DatabaseService` façade 共同提供稳定契约；`/api/annotators` 仍只管理 Argilla 外部身份映射。
...
| HTTP-01 | 平台成员管理页和 HTTP 生命周期 | 提供显式成员列表、邀请、change/revoke、最后 admin、审计和幂等响应契约；请求 actor/caller 由服务端注入 | `tests/test_workspace_members.py`、`tests/test_panel_control.py`、`tests/test_panel_login.py` | 已覆盖 |
...
平台成员由成员管理页和 `/api/members` 负责。邀请按邮箱匹配，但最终 principal 仍由认证后的 `(issuer, subject)` 确定；成员管理页操作 workspace role，并在变更后使后续请求立即重新判权。`/api/annotators`、`/api/cohorts` 只能管理 Argilla 映射、外部验证和人员组，不能被复用为平台成员管理页。

## 当前阻塞点与后端验收建议

1. 成员管理 HTTP API 已提供列表、邀请、change、revoke、workspace scope、资源不可见语义、`Idempotency-Key`、actor/caller 服务端注入和审计结果；发布时必须继续以这些契约验收。
2. 后端必须把 active workspace membership 作为 `authorize_task` 的硬前置条件。撤销 workspace membership 后，即使 task ACL 行仍残留，也必须立即拒绝 task 访问；不能让前端自行删除 task ACL 来弥补授权缺口。
...
4. 浏览器到 R2 的端到端测试仍需在部署环境运行；本地契约测试覆盖 `DatabaseService`、Cloudflare auth、成员/Argilla 控制路由和既有数据湖边界。
